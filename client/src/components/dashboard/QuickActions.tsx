    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {actions.map((action) => (
                <Card
                    key={action.label}
                    className="cursor-pointer border-2 border-transparent hover:border-primary/20 hover:shadow-md transition-all group overflow-hidden"
                    onClick={() => onSelect({ domain: action.domain, difficulty: action.difficulty, duration: action.duration })}
                >
                    <div className="p-4 flex flex-col items-center gap-3 text-center">
                        <div className={cn("p-3 rounded-full transition-transform group-hover:scale-110", action.bgColor)}>
